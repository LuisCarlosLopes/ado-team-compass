...
1. Instale este bundle conforme o procedimento do Google Antigravity.
2. O manifesto deste host é mínimo por decisão de compatibilidade: registre o servidor na configuração de MCP do Antigravity, com o comando `python3` e o caminho absoluto de `bin/atc-mcp.py` dentro do bundle instalado. O arquivo `mcp-server.json` do bundle traz a entrada pronta para copiar.
3. Configure o servidor MCP oficial do Azure DevOps na configuração de MCP do Antigravity, apontando para o servidor remoto oficial da organização.
